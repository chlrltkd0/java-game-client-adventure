...
public class MainCharacter implements Observee {
	...
	private List<CItem> itemList = new LinkedList<>();
	private CItem weapon = null;
	...
	public void gainItem() {
		if(itemList.size()<48) {
			for(CItem item : Client.getInstance().getGameManager().getCurrentMap().getItemList()) {
				if(this.getBounds().contains(new Point(item.getX(), item.getY()))){
					Client.getInstance().getGameManager().gatherItem(item.getItemVO());
					break;
				}
			}
		}
	...
	public void equipWeapon(CItem item) {
		unEquipWeapon();
	...
	public void upStat(int type) {
		if(characterVO.getChr_statPoint()>0) {
			...
			Client.getInstance().getGameManager().upStat(integerVO);
		}
	}
	public int getStr() {
	...
	public void autoMove() {
		if(characterVO.isWalking() && !characterVO.isAttacking() && !characterVO.isDead()) {
			...
			if(0 < nextX && nextX < Client.getInstance().getGameManager().getCurrentMap().getMap_width()-64)
				characterVO.setChr_x(nextX);
			if(0 < nextY && nextY < Client.getInstance().getGameManager().getCurrentMap().getMap_height()-64)
	...
	public CItem getWeapon() {
		return weapon;
	}

	public void setWeapon(CItem weapon) {
		this.weapon = weapon;
	}
	...
	public void setItemList(List<CItem> itemList) {
