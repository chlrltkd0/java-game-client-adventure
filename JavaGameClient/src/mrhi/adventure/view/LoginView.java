...
//		uni.addGlyphs(1, 65536);
	...
	public void update(GameContainer gameContainer, StateBasedGame arg1, int arg2) throws SlickException {
		...
		if(Client.getInstance().getAccountManager().getAccount()!=null) {
			Client.getInstance().getCharacterManager().requestCharacterList();
		...
		if(input.isMousePressed(0)) {
			if(idField.getX() < input.getMouseX() && idField.getX()+idField.getWidth() > input.getMouseX()
				...
				Client.getInstance().getAccountManager().login(accountVO);
				idField.setText("");
				pwField.setText("");
			}
