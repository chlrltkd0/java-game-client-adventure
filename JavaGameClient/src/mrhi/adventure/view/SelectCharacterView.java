...
public class SelectCharacterView extends BasicGameState {
	...
	public void render(GameContainer gameContainer, StateBasedGame arg1, Graphics g) throws SlickException {
		...
		for(int i=0; i<Client.getInstance().getCharacterManager().getCharacterList().size(); i++) {
			CharacterVO charVO = Client.getInstance().getCharacterManager().getCharacterList().get(i);
		...
		if(deleteCharacter) {
			...
			g.drawString(Client.getInstance().getCharacterManager().getCharacterList().get(select).getChr_name(), 350, 180);
		}
	}
		...
		if(input.isMousePressed(0)) {
			if(deleteCharacter) {
				if(340 < input.getMouseX() && 380 > input.getMouseX() && 247 < input.getMouseY() && 270 > input.getMouseY()) {
					CharacterVO charVO = Client.getInstance().getCharacterManager().getCharacterList().get(select);
					Client.getInstance().getCharacterManager().deleteCharacter(charVO);
					Client.getInstance().getCharacterManager().getCharacterList().remove(charVO);
				...
				charVO.setChr_idx(Client.getInstance().getCharacterManager().getCharacterList().get(select).getChr_idx());
				Client.getInstance().getCharacterManager().requestCharacter(charVO);
				
				MapVO mapVO= new MapVO();
				mapVO.setMap_idx(1000);
				Client.getInstance().getGameManager().requestMap(mapVO);
				
				Client.getInstance().getGameManager().requestItemList();
				...
				Client.getInstance().getGameManager().startTimeElapse();
			}
			else if(663 < input.getMouseX() && 716 > input.getMouseX() && 10 < input.getMouseY() && 35 > input.getMouseY()) {
				Client.getInstance().getAccountManager().setAccount(null);
